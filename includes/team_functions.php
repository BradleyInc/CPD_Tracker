 ...
 */
function getTeamMemberCount($pdo, $team_id) {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM user_teams WHERE team_id = ?");
    $stmt->execute([$team_id]);
    return $stmt->fetchColumn();
}

/**
 * Check if user can access team (is admin or team member)
 */
function canUserAccessTeam($pdo, $user_id, $team_id) {
